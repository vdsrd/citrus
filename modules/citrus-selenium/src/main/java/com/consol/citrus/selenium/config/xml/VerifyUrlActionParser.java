...
import com.consol.citrus.selenium.action.VerifyUrlAction;

/**
 * Action that verifies the current URL.
 * 
 * @author VASCO Data Security
 * @since 2.7
 */
public class VerifyUrlActionParser extends AbstractWebActionParser {
    ...
    protected void doParse(Element element, BeanDefinitionBuilder builder) {
        String urlPath = element.getAttribute("path").trim();
        if (urlPath == null || urlPath.isEmpty()) {
            throw new CitrusRuntimeException("This action requires the path attribute");
        }
        builder.addPropertyValue("path", urlPath);
    }
