...
import com.consol.citrus.selenium.action.SetCookieAction;

/**
 * @author VASCO Data Security
 * @since 2.7
 */
public class SetCookieActionParser extends AbstractWebActionParser {
    ...
    protected void doParse(Element element, BeanDefinitionBuilder builder) {
        String cookieName = element.getAttribute("cookie").trim();
        String cookieValue = element.getAttribute("value").trim();
        if (StringUtils.hasText(cookieName)) {
            BeanDefinitionParserUtils.setPropertyValue(builder, cookieName, "cookie");
        }
        if (StringUtils.hasText(cookieValue)) {
            BeanDefinitionParserUtils.setPropertyValue(builder, cookieValue, "value");
        }
    }
